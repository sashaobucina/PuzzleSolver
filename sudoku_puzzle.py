...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Override fail_fast
        If there is one open position that has no symbols
        available to put in it. If there is one open position
        where the symbols already used in the same row, column,
        and subsquare exhaust the symbols available, there is
        ...
        >>> s = SudokuPuzzle(9, grid, symbol_set )
        >>> s.fail_fast()
        False
        >>> grid = ["A", "B", "D", "*"]
        >>> grid += ["C", "D", "A", "B"]
        ...
        # return indices for each star in self._symbols
...
if __name__ == "__main__":
    ...
    # print("Solving sudoku puzzle from http://www.puzzlemadness.co.uk/16by16giantsudoku/ on May 1 2017... \n\n{}\n\n".format(s))
    ...
    #     sol = sol.children[0]
    # end = time()
    # print("Time to solve 16x16 using depth_first: {} seconds \n".format(end - start))
    # print(sol)
